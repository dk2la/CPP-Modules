...
int		main() {
	...
	Intern intern;
	try {
		Form* somethingShit = intern.makeForm("Random shit", "Random man");
		std::cout << *somethingShit << std::endl;
		delete somethingShit;
	}
	catch (const std::exception& e) {
		std::cout << e.what() << std::endl;
	}
	try {
		Form* actualForm = intern.makeForm("Shrubbery Creation", "Chelick");
		std::cout << *actualForm << std::endl;
		delete actualForm;
	}
	catch (const std::exception& e) {
		std::cout << e.what() << std::endl;
